...
pub fn get_runtime_compute_cap() -> Result<usize, anyhow::Error> {
    ...
    let dev = driver::result::device::get(0).context("Could not get CUDA device 0")?;

    let major = unsafe {
        driver::result::device::get_attribute(dev, CU_DEVICE_ATTRIBUTE_COMPUTE_CAPABILITY_MAJOR)
    }
    .context("Could not retrieve device compute capability major")?;

    let minor = unsafe {
        driver::result::device::get_attribute(dev, CU_DEVICE_ATTRIBUTE_COMPUTE_CAPABILITY_MINOR)
    }
