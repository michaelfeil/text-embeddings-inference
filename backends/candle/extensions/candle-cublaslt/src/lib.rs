...
pub fn fused_matmul(
    ...
    if let Some(bias) = bias {
        a.apply_op3_no_bwd(b, bias, &op)
    } else {
        a.apply_op2_no_bwd(b, &op)
    }
}
...
mod tests {
    ...
    #[test]
    fn bf16_fused_matmul_bias_relu() -> Result<()> {
        let device = Device::Cuda(candle::CudaDevice::new_with_stream(0)?);
        let a = Tensor::from_vec(
            (0..128).map(|i| (i % 7) as f32 - 3.).collect::<Vec<_>>(),
            (8, 16),
            &device,
        )?
        .to_dtype(DType::BF16)?;
        let b = Tensor::from_vec(
            (0..32).map(|i| (i % 5) as f32 - 2.).collect::<Vec<_>>(),
            (2, 16),
            &device,
        )?
        .to_dtype(DType::BF16)?;
        let bias = Tensor::full(0.5f32, 8, &device)?.to_dtype(DType::BF16)?;
        let result = fused_matmul(
            &a,
            &b,
            None,
            None,
        ...
        let expected = b
            ...
            .to_dtype(DType::BF16)?;
        assert_eq!(
            result.flatten_all()?.to_vec1::<bf16>()?,
            expected.flatten_all()?.to_vec1::<bf16>()?
        );
        Ok(())
    }
