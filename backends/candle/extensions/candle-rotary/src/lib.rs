...
mod tests {
    ...
    #[test]
    fn bf16_rotary_matches_neox_reference() -> Result<()> {
        let device = Device::Cuda(candle::CudaDevice::new_with_stream(0)?);
        let values: Vec<f32> = (0..64).map(|i| (i as f32 - 32.) / 16.).collect();
        let q = Tensor::from_vec(values.clone(), (2, 2, 16), &device)?.to_dtype(DType::BF16)?;
        let k = q.copy()?;
        let cos = Tensor::full(0.5f32, (2, 8), &device)?.to_dtype(DType::BF16)?;
        let sin = Tensor::full(0.25f32, (2, 8), &device)?.to_dtype(DType::BF16)?;
        apply_rotary_inplace(&q, &k, &cos, &sin, true)?;
        let actual = q.to_dtype(DType::F32)?.flatten_all()?.to_vec1::<f32>()?;
        for (i, &v) in actual.iter().enumerate() {
            let dim = i % 16;
            let (other, sign) = if dim < 8 { (i + 8, -1.) } else { (i - 8, 1.) };
            let expected = bf16::from_f32(values[i] * 0.5 + sign * values[other] * 0.25).to_f32();
            assert_eq!(v, expected);
        }
        assert_eq!(
            actual,
            k.to_dtype(DType::F32)?.flatten_all()?.to_vec1::<f32>()?
        );
        Ok(())
    }
}
