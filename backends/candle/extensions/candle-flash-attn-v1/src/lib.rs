...
impl FlashAttnVarLen {
    ...
    >(
        ...
        let mut softmax_lse = dev.alloc_zeros::<f32>(batch_size * num_heads * self.max_seqlen_q)?;
        ...
        let mut dst_temp = if max_seqlen_k_rounded > blocksize_c {
        ...
        unsafe {
            let q_ptr = ({
                let (ptr, guard) = q.device_ptr(&stream);
                guards.push(guard);
                ptr
            }) as *const core::ffi::c_void;
            let k_ptr = ({
                let (ptr, guard) = k.device_ptr(&stream);
                guards.push(guard);
                ptr
            }) as *const core::ffi::c_void;
            let v_ptr = ({
            ...
            let dst_tmp_ptr = if let Some(slice) = &mut dst_temp {
                ({
            ...
            let softmax_lse_ptr = ({
                let (ptr, guard) = softmax_lse.device_ptr_mut(&stream);
                guards.push(guard);
                ptr
            }) as *const core::ffi::c_void;
            let seqlens_q_ptr = ({
            ...
            let seqlens_k_ptr = ({
                let (ptr, guard) = seqlens_k.device_ptr(&stream);
                guards.push(guard);
                ptr
            }) as *const core::ffi::c_int;
            ffi::run_mha(
