...
fn flash_attn_varlen() -> Result<()> {
    ...
    let ys = {
        ...
        candle_flash_attn_v1::flash_attn_varlen(
...
#[test]
fn causal_attention_on_nondefault_stream() -> Result<()> {
    let device = Device::Cuda(candle::CudaDevice::new_with_stream(0)?);
    for dtype in [DType::F16, DType::BF16] {
        let q = Tensor::zeros((3, 1, 32), dtype, &device)?;
        let k = Tensor::zeros((3, 1, 32), dtype, &device)?;
        let v = (Tensor::arange(0u32, 96, &device)?.to_dtype(DType::F32)? / 32.)?
            .reshape((3, 1, 32))?
            .to_dtype(dtype)?;
        let lengths = Tensor::new(&[0u32, 3], &device)?;
        let actual = candle_flash_attn_v1::flash_attn_varlen(
            &q, &k, &v, &lengths, &lengths, 3, 3, 0.5, true,
        )?
        .to_dtype(DType::F32)?
        .flatten_all()?
        .to_vec1::<f32>()?;
        for (i, value) in actual.iter().enumerate() {
            let expected = (i / 32) as f32 / 2. + (i % 32) as f32 / 32.;
            assert!(
                (value - expected).abs() < 0.016,
                "{dtype:?} row value {i}: {value} vs {expected}"
            );
        }
    }
    Ok(())
